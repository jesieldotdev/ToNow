...
export const setTask =
  (...[, , , dispatch]: any) =>
  <T extends FlattenKeys<TaskState>>(field: T, value: DeepType<TaskState, T>): void =>
    dispatch(setTaskTask(field, value));

export const setTaskItem =
  (getState: () => RootState, actions: ActionsType) =>
  <T extends FlattenKeys<TaskItem>>(field: T, value: DeepType<TaskItem, T>): void => {
    const {
      task: { setTask }
    } = actions;
...
export const getTasks =
  (_getState: () => RootState, actions: ActionsType) => (searchFilter?: SearchFilter<TaskItem>) =>
...
export const filterTasks = (getState: () => RootState, actions: ActionsType) => () =>
