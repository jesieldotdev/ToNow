...
export const setSetting =
  (...[, , , dispatch]: any) =>
  <T extends FlattenKeys<SettingState>>(field: T, value: DeepType<SettingState, T>): void =>
    dispatch(setSettingSetting(field, value));

export const setSettingItem =
  (getState: () => RootState, actions: ActionsType) =>
  <T extends FlattenKeys<SettingItem>>(field: T, value: DeepType<SettingItem, T>): void => {
    const {
      setting: { setSetting },
    } = actions;
...
export const filterSettings = (getState: () => RootState, actions: ActionsType) => () =>
...
export const toggleTheme = (getState: () => RootState, actions: ActionsType) => (): void => {
  const {
    setting: { setSetting },
  } = actions;
